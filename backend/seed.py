...
# Additional startup profiles gathered from top global + Indian incubators
# (2020-2026 cohorts). Generated by workspace/startup-intel-data/convert.py —
# edit the source JSONs there and re-run the converter, don't hand-edit this file.
import json as _json
import os as _os

_DATA_FILE = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "data", "startups.json")
if _os.path.exists(_DATA_FILE):
    with open(_DATA_FILE) as _f:
        _extra = _json.load(_f)
    _existing = {p["name"].lower() for p in STARTUP_PROFILES}
    for _p in _extra:
        if _p["name"].lower() not in _existing:
            STARTUP_PROFILES.append(_p)
    print(f"Loaded {len(_extra)} incubator-sourced startup profiles from data/startups.json")
